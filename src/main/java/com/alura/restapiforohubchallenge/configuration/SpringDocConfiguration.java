package com.alura.restapiforohubchallenge.configuration;

import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.Components;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.License;
import org.springframework.context.annotation.Bean;
import io.swagger.v3.oas.models.security.SecurityScheme;
import org.springframework.context.annotation.Configuration;

@Configuration
public class SpringDocConfiguration {
    @Bean
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .components(new Components()
                        .addSecuritySchemes("bearer-key",
                                new SecurityScheme()
                                        .type(SecurityScheme.Type.HTTP)
                                        .scheme("bearer")
                                        .bearerFormat("JWT"))
                )
                // Para personalizar la documentacion generada por swagger spring doc.
                .info(new Info()
                        .title("REST API ForumHub")
                        .description("This project is a REST API for a forum where participants\n" +
                                "of a platform can post their questions about certain topics.\n" +
                                "This API was developed in Java with Spring Boot and a \n" +
                                "PostgreSQL database and is part of the specialization in \n" +
                                "backend with Java and Spring Framework of the ONE - Oracle\n" +
                                "Next Education program.")
                        .contact(new Contact()
                                .name("Backend Team")
                                .email("backend@forumhub.alura"))
                        .license(new License()
                                .name("Apache 2.0")
                                .url("http://forumhub.alura/api/licence")));
    }
}
